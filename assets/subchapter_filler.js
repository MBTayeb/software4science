  ...
  const core = {
    ...
    processParentNavigation: async (manifest, parentPath, parentTitleRaw) => {
      const container = document.querySelector(
        CONFIG.SELECTORS.parentNavContainer,
      );
      if (!container || !parentPath) return;

      const {prev, next} = ManifestNav.getParentChapterNav(
        manifest,
        parentPath,
      );
      ...
      const [prevTitle, nextTitle] = await Promise.all([
        prev
          ? ManifestNav.fetchText(`../../${prev}title.txt`, true)
          : Promise.resolve(null),
        next
          ? ManifestNav.fetchText(`../../${next}title.txt`, true)
          : Promise.resolve(null),
      ]);
    ...
    loadSubpageNavigation: async (manifest, parentPath, parentTitleRaw) => {
      const subpageNav = document.querySelector(
        CONFIG.SELECTORS.subpageNavigation,
      );
      ...
      try {
        const resources = await Promise.all(
          CONFIG.CRITICAL_RESOURCES.map((resource) =>
            utils
              .fetchResource(resource.url, CONFIG.ERROR_MESSAGES.loadFailed)
              .then((content) => ({[resource.name]: content})),
          ),
        ).then((results) => Object.assign({}, ...results));

        const contentPlaceholder = document.querySelector(
          CONFIG.SELECTORS.contentPlaceholder,
        );
        if (contentPlaceholder) {
          contentPlaceholder.insertAdjacentHTML(
            "afterbegin",
        ...
        if (h1)
          h1.innerHTML = `<a href="../">${resources.parentTitle.trim()}</a>`;

        const manifest = await ManifestNav.getManifest("../../");
        const {parentPath} = ManifestNav.getSubPrevNext(manifest);

        await core.processParentNavigation(
          manifest,
          parentPath,
          resources.parentTitle,
        );
